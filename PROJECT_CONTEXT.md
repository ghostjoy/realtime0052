...
- `YTD股利型ETF`：2026 年截至今日全部台股股利/配息型 ETF（卡片頁）
- `YTD top15 ETF`：2026 年截至今日前15大台股 ETF（卡片頁）
- `2025 後20大最差勁 ETF`：2025 全年區間「輸給台股大盤(%)」最多的後20名台股 ETF（排除復權事件，卡片頁）
- `共識代表 ETF`：以前10 ETF 成分股交集推導最具代表性的單一 ETF（附前3備選）
- `兩檔 ETF 推薦`：以共識代表（核心）+ 低重疊動能（衛星）輸出兩檔可執行組合
- `YTD Active ETF`：台股 Active ETF 的 2026 YTD Buy & Hold 績效卡，含 Benchmark 對照圖（策略/基準/各ETF買進持有）
...
- `00735 熱力圖`：00735 成分股逐檔回測，對大盤做超額報酬熱力圖
...
- `services/market_data_service.py`
  ...
  - Benchmark / ETF 成分股來源整合（`00910 / 00935 / 00735 / 00993A / 0050 / 0052`）
...
- 新增/擴充 ETF 熱力圖分頁：`00910`（含全球成分股 YTD 分組）、`00935`、`00735`、`00993A`、`0050`、`0052`
- 成分股清單改為本地 DuckDB 快取，避免反覆抓取
- 回測工作台新增「回測前自動補資料缺口」，可針對缺口標的增量回補
- 新增三個 ETF 排行卡片頁：`2026 YTD 前十大股利型、配息型 ETF`、`2026 YTD 前十大 ETF`、`2025 後20大最差勁 ETF`（TWSE 快照區間報酬）
