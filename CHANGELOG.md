...
- `2026 YTD` 兩張排行卡片調整為 Top15：
  ...
  - `前10檔數 / 前10占比`、卡片標題、功能地圖與教學文案同步調整
- ETF 分類篩選統一改為使用本專案 `類型` 欄位：
  - `2026 YTD 前十大股利型、配息型 ETF` 改為依 `類型 = 股利型` 建立母體，不再直接依賴 `官方次分類`
  - `2026 YTD 主動式 ETF` 改為依 `類型 = 主動式` 建立母體，不再直接依賴 `官方主分類`
  - 相關頁面說明文案同步改為以本專案 `類型` 為準
- `台股 ETF 全類型總表`、`2026 YTD 前十大股利型、配息型 ETF`、`2026 YTD 主動式 ETF` 改採 TWSE ETF e添富官方分類：
  ...
  - 高股息主題頁改依官方 `主題/因子` 的 `高股息 / 高息低波動` 篩選
...
- 修正 `export-tw-etf-report` 無法單獨輸出 constituent heatmap 的問題：
  - 新增 `--heatmap-only` 模式，可直接輸出單一成分股熱力圖 PNG
  - `--out` 可直接指定 `.png` 檔名，或指定目錄後自動命名
...
- 修正 `2026 YTD 前十大 ETF` 與 `2026 YTD 前十大股利型、配息型 ETF` 的 `離開排行` 顯示：
  - 若前一交易日沒有新離榜標的，不再誤顯示 `資料不可比對`
  - 會沿用最近一次有離榜事件的結果持續顯示，直到下一次離榜更新
  - 若往前回看皆無離榜，改顯示 `近期無離榜 ETF`
- 調整 `筆記本` 版面與操作：
...
- `daily_bars` 新增持久化 `vwap` 欄位（DuckDB/Parquet 與 legacy SQLite 皆支援）：
  - 新寫入或同步的日 K 資料會自動計算並保存 `vwap`。
  - 舊資料若尚未有 `vwap`，讀取時會回補計算，避免既有資料直接失效。
