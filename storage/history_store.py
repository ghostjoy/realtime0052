...
class HistoryStore:
        ...
        with self._connect_ctx() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS heatmap_hub_entries (
                    ...
                    pin_as_card INTEGER NOT NULL DEFAULT 0,
                ...
                CREATE TABLE IF NOT EXISTS tw_etf_super_export_runs (
                    ...
                    run_id TEXT NOT NULL UNIQUE,
                    created_at TEXT NOT NULL,
                    ytd_start TEXT NOT NULL,
                    ytd_end TEXT NOT NULL,
                    compare_start TEXT NOT NULL,
                    compare_end TEXT NOT NULL,
                    trade_date_anchor TEXT NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_heatmap_hub_entries_pin_updated
                    ON heatmap_hub_entries(pin_as_card DESC, updated_at DESC);
                ...
                CREATE INDEX IF NOT EXISTS idx_tw_etf_super_export_runs_created_at
                    ON tw_etf_super_export_runs(created_at DESC);
                ...
                CREATE INDEX IF NOT EXISTS idx_tw_etf_mis_daily_trade_date
                    ON tw_etf_mis_daily(trade_date DESC);
        ...
        with self._connect_ctx() as conn:
            conn.execute(
                ...
                (
                    run_id,
                    now.isoformat(),
                    str(ytd_start or "").strip(),
                    str(ytd_end or "").strip(),
                    str(compare_start or "").strip(),
                    str(compare_end or "").strip(),
                    str(trade_date_anchor or "").strip(),
    ...
    def load_latest_tw_etf_super_export_run(self) -> SuperExportRun | None:
        ...
        return SuperExportRun(
            run_id=str(row[0] or ""),
            created_at=created_at,
            ytd_start=str(row[2] or ""),
            ytd_end=str(row[3] or ""),
            compare_start=str(row[4] or ""),
            compare_end=str(row[5] or ""),
            trade_date_anchor=str(row[6] or ""),
            output_path=str(row[7] or ""),
        ...
        with self._connect_ctx() as conn:
            for code, values in dedup.items():
                conn.execute(
                    ...
                    ON CONFLICT(etf_code, trade_date) DO UPDATE SET
                        etf_name=excluded.etf_name,
                        issued_units=excluded.issued_units,
                        creation_redemption_diff=excluded.creation_redemption_diff,
    ...
    def upsert_heatmap_hub_entry(
        ...
        *,
        ...
        with self._connect_ctx() as conn:
            ...
            existing_last_opened = self._normalize_text(row[3])
            ...
            next_last_opened = now if bool(opened) else existing_last_opened
            ...
            conn.execute(
                ...
                WHERE etf_code=?
                """,
                (next_name, next_pin, next_open, next_last_opened, created_at, now, code),
            )
