...
- `YTD Active ETF`：台股 Active ETF 的 2026 YTD Buy & Hold 績效卡，依本專案 `類型 = 主動式` 建立母體，並排除非 ETF 的 `A` 尾碼股票，含 Benchmark 對照圖
...
4. 想看主動式 ETF 就到 `YTD Active ETF`，同樣可用 `更新最新市況` + Benchmark 對照卡。
